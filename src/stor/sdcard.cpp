...
namespace Emu293 {
	...
	const uint32_t reg_cid[4] = { 0x42445345, 0x6D753239, 0x10000000, 0x0000F7FF };//most significant word first
	//this is populated with size at runtime, but we can prepopulate some stuff beforehand
	static uint32_t reg_csd[4] = { 0x0008015A, 0x5B5BE000, 0x0000C000, 0x02E000FF };//most significant word first
	...
	void SD_Command(uint8_t command, uint32_t argument) {
		...
		if(currentState == SD_STATE_INACTIVE) return;
		using namespace CMD;
		clear_bit(currentCardStatus, cardStatus_iglCommand);

		bool isAddressed = (currentState == SD_STATE_STDBY) || (currentState == SD_STATE_TRANS)|| (currentState == SD_STATE_SEND) ||  (currentState == SD_STATE_RECV)  ||  (currentState == SD_STATE_PROG) || (currentState == SD_STATE_DIS);
		...
		if(!check_bit(currentCardStatus, cardStatus_appCmd)) {
			...
			case SELECT_CARD: {
				if((argument >> 16)  == RCA) {
					...
					if((currentState == SD_STATE_STDBY) ||  (currentState == SD_STATE_TRANS)||  (currentState == SD_STATE_SEND))
						currentState = SD_STATE_STDBY;
