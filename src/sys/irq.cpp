...
namespace Emu293 {
	...
	void ProcessInterrupts() {
		if(interruptsEnabled) {
			//TODO: interrupt priorities (although the standard D99+ firmware does not use them)
			for(int i = 0; i < 32; i++) {
				if(check_bit(int_regs[INT_PNDL],i) && !check_bit(int_regs[INT_MASKL],i) && !intsFired[63 - i]) {
					//printf("INT %d!\n",63 - i);
			...
			for(int i = 0; i < 32; i++) {
				if(check_bit(int_regs[INT_PNDH],i) && !check_bit(int_regs[INT_MASKH],i)&& !intsFired[31 - i] ) {
					//printf("INT %d!\n",31 - i);
	...
	const Peripheral IRQPeripheral = {
			"PIC",
			InitIRQDevice,
			IRQDeviceReadHandler,
